# New reward strategy
# No explicit reward for interacting with things in the environment
# Only associate the rewards with a general goal
# e.g. +1 for seeing a hider, -1 if you are being seen

# In our case:
# -1 if health is below 50% (x%)
# +1 if health is above 70% (y%)

# Health = food_amount + sleep_amount
# Collecting food: large food_amount increase when collected
# Sleeping: slow sleep_amount increase over time when in 'sleep' zone
# Moving around: sleep and food each decrease gradually over time

# Future variant: moving around speed is slower if health is low
# add 2 blocks so agents have option to block doors to the room w/ food in it
...
from modules.wall import RandomWalls,WallScenarios
...
from wrappers.sleep import AgentHealthWrapper, SleepHealthWrapper, AlwaysSleepWrapper, AltFoodHealthWrapper
...
class BaselineRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        this_rew = np.subtract(np.ones((self.n_agents,)), 1.5)
        
        rew = np.zeros((self.n_agents,)) + rew
        return obs, rew, done, info
...
def make_env(n_agents=2, n_food=10, horizon=50, floor_size=4.,
             n_lidar_per_agent=8, agent_health=True, visualize_lidar=True, compress_lidar_scale=None,
             grid_size=50,door_size=4,scenario='quadrant'):

    env = SleepVariant(horizon=horizon, grid_size=grid_size,floor_size=floor_size, n_agents=n_agents, n_food=n_food)
    ...
    env.add_module(Agents(n_agents,color=[np.array((152., 251.,152., 255.)) / 255,np.array((255., 69., 0., 255.)) / 255],placement_fn=agent_placement_fn))
    # Add sleep sites
    ...
    if agent_health:
        env = SleepHealthWrapper(env)
        env = AlwaysSleepWrapper(env, agent_idx_allowed=np.arange(n_agents))
        env = AltFoodHealthWrapper(env)
